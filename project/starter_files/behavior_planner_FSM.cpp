...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = velocity_mag * velocity_mag / (2*accel_mag); // d=v^2/2a
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += _stop_line_buffer * std::cos(ang);  // <- Fix This: cos(ang) along x axis
      goal.location.y += _stop_line_buffer * std::sin(ang);  // <- Fix This: sing(ang) along y axis

      // LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
      //          << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      // speed should be 0 when stopped
    ...
    goal = _goal;  // <- Fix This: set as the previous goal
    ...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      ...
      _active_maneuver = STOPPED ;  // <- Fix This: Use distance to set state
